...
@push('header-action')
    @if (
        ...
        <x-core::button tag="a" :href="route('plugins.new')" color="primary" icon="ti ti-plus" class="ms-auto">
...
@if ($plugins->isNotEmpty())
    <div class="d-flex gap-2 justify-content-between">
        <div class="w-100 w-sm-25">
            <x-core::form.text-input type="search" name="search"
                :placeholder="trans('packages/plugin-management::plugin.search')" :group-flat="true"
            ...
            <div class="d-block d-sm-none dropdown">
                <x-core::button class="dropdown-toggle" data-bs-toggle="dropdown">
                    <span data-bb-toggle="status-filter-label" class="ms-1">
                        ...
                        (<span data-bb-toggle="plugins-count"
                            data-status="{{ array_key_first($filterStatuses) }}">{{ $plugins->count() }}</span>)
                    </span>
                </x-core::button>
                <div class="dropdown-menu dropdown-menu-end" data-popper-placement="bottom-end">
                    @foreach ($filterStatuses as $key => $value)
                        <button @class(['dropdown-item', 'active' => $loop->first]) type="button" data-value="{{ $key }}"
            ...
            <div class="d-none d-sm-flex form-selectgroup">
                @foreach ($filterStatuses as $key => $value)
                    <label class="form-selectgroup-item">
                        <input type="radio" name="status" value="{{ $key }}" data-bb-toggle="change-filter-plugin-status"
                            class="form-selectgroup-input" @checked($loop->first) />
    ...
    <div class="row row-cols-2 row-cols-sm-3 row-cols-lg-4 row-cards plugin-list">
        @foreach ($plugins as $plugin)
            <div class="col plugin-item" data-name="{{ $plugin->name }}" data-author="{{ $plugin->author }}"
                ...
                <x-core::card class="h-100">
                    ...
                    <x-core::card.body class="d-flex flex-column justify-content-between">
                            ...
                            @if ($plugin->description)
                                <p class="text-secondary " title="{{ $plugin->description }}">
                                    {{ $plugin->description }}
                                </p>
                            @endif
                    ...
                    <x-core::card.footer>
                        <div class="btn-list">
                            @if (auth()->user()->hasPermission('plugins.edit'))
                                <x-core::button type="button" :color="$plugin->status ? 'warning' : 'primary'"
                                    class="btn-trigger-change-status" data-plugin="{{ $plugin->path }}"
...
<x-core::empty-state :title="trans('No plugins found')" :subtitle="trans('It looks as there are no plugins here.')"
    @style(['display: none' => $plugins->isNotEmpty()]) />
@stop

@push('footer')
    <x-core::modal.action id="remove-plugin-modal" type="danger"
    ...
    @if ($isEnabledMarketplaceFeature)
        <x-core::modal id="confirm-install-plugin-modal" :title="trans('packages/plugin-management::plugin.install_plugin')"
            button-id="confirm-install-plugin-button" :button-label="trans('packages/plugin-management::plugin.install')">
